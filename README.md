...
**Capstone Overview:**
  ...
  2: _Three Laptops_ - duplicate listings for the same product because the update products method was calling on CREATE instead of UPDATE, therefore it was adding a new listing when it should have been editing the already existing one.

 ![easyShopLogin1](https://github.com/paulmlaviwa/EasyShop/assets/146879419/c6990c86-720d-436a-9ec6-010c588e88a5)  
 
**Implemented Methods: **
_Categories Controller_ - getProductsById(by category Id), addCatgegory(Admin ONLY authorization), updateCategory(Admin ONLY authorization), deleteCategory(Admin ONLY authorization), 
_MySQLCategories_ - getAllCategories(), getById(by category Id), createCategory(Admin ONLY authorization), updateCategory(uses the category Id), deleteCategory(uses the cat id)
